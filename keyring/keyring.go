...
// ItemUpdate updates an item
func ItemUpdate(query Item, item Item) error {
	queryDict, err := ConvertMapToCFDictionary(query.attr)
	if err != nil {
		return err
	}
	defer Release(C.CFTypeRef(queryDict))

	itemDict, err := ConvertMapToCFDictionary(item.attr)
	if err != nil {
		return err
	}
	defer Release(C.CFTypeRef(itemDict))

	errCode := C.SecItemUpdate(queryDict, itemDict)
	return checkError(errCode)
}
